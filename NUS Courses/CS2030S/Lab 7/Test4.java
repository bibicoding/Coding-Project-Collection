...
/**
 * Test 4 for CS2030S Exercise 7 (AY24/25 Sem 2).  Tests
...
class Test4 {
  ...
  public static void main(String[] args) {
    ...
    i.expectReturn(
        "InfiniteList.sentinel().isSentinel()", () -> InfiniteList.sentinel().isSentinel(), true);
    i.expectReturn(
        "InfiniteList.sentinel().map(x -> 2).isSentinel()",
        () -> InfiniteList.sentinel().map(x -> 2).isSentinel(),
        true);
    i.expectReturn(
        "InfiniteList.sentinel().filter(x -> true).isSentinel()",
        () -> InfiniteList.sentinel().filter(x -> true).isSentinel(),
        true);
    i.expectReturn(
        "InfiniteList.sentinel().filter(x -> false).isSentinel()",
        () -> InfiniteList.sentinel().filter(x -> false).isSentinel(),
        true);
    ...
    i.expectReturn(
        ...
        "[? ?]");
    i.expectReturn(
        "InfiniteList.iterate(1, x -> x + 1).limit(4).toString()",
        () -> InfiniteList.iterate(1, incr).limit(4).toString(),
        "[[1] ?]");
    i.expectException(
        "InfiniteList.iterate(1, x -> x + 1).limit(0).head()",
        () -> InfiniteList.iterate(1, incr).limit(0).head(),
        new java.util.NoSuchElementException());
    i.expectReturn(
        "InfiniteList.iterate(1, x -> x + 1).limit(1).head()",
        () -> InfiniteList.iterate(1, incr).limit(1).head(),
        1);
    i.expectReturn(
        "InfiniteList.iterate(1, x -> x + 1).limit(4).head()",
        () -> InfiniteList.iterate(1, incr).limit(4).head(),
        1);
    i.expectException(
        "InfiniteList.iterate(1, x -> x + 1).limit(1).tail().head()",
        () -> InfiniteList.iterate(1, incr).limit(1).tail().head(),
        new java.util.NoSuchElementException());
    i.expectReturn(
        "InfiniteList.iterate(1, x -> x + 1).limit(4).tail().tail().head()",
        () -> InfiniteList.iterate(1, incr).limit(4).tail().tail().head(),
        3);
    i.expectException(
        "InfiniteList.iterate(1, x -> x + 1).limit(4).limit(1).tail().head()",
        () -> InfiniteList.iterate(1, incr).limit(4).limit(1).tail().head(),
        new java.util.NoSuchElementException());
    i.expectException(
        "InfiniteList.iterate(1, x -> x + 1).limit(1).limit(4).tail().head()",
    ...
    i.expectReturn(
        ...
        () -> InfiniteList.iterate("A", s -> s + "Z").map(s -> s.length()).head(),
        1);
    i.expectReturn(
        "InfiniteList.iterate(\"A\", s -> s + \"Z\").map(s -> s.length()).limit(2).tail().head()",
        () -> InfiniteList.iterate("A", s -> s + "Z").map(s -> s.length()).tail().head(),
        2);
    i.expectException(
    ...
    i.expectReturn(
        "InfiniteList.generate(() -> null).limit(4).limit(1).toList()",
        () -> InfiniteList.<Object>generate(() -> null).limit(4).limit(1).toList(),
        Arrays.asList(new Object[] {null}));
    i.expectReturn(
        "InfiniteList.generate(() -> null).limit(1).limit(4).toList()",
