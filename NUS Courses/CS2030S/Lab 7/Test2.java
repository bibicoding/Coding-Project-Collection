...
/**
 * Test 2 for CS2030S Exercise 7 (AY24/25 Sem 2).  Tests
...
class Test2 {
  ...
  public static void main(String[] args) {
    ...
    i.expect(
        "InfiniteList.generate(() -> 1).map(x -> x * 2).tail().head()\n" + " ..returns 2",
        () -> InfiniteList.generate(generator).map(doubler).tail().head(),
        2);
    i.expect(" ..causes two evals of () -> 1", generateHistory, List.of(1, 1));
    i.expect(" ..causes two evals of x -> x * 2", doublerHistory, List.of(1, 1));
    ...
    twos.tail().head();
    i.expect(
        "InfiniteList<Integer> ones = InfiniteList.generate(() -> 1)\n"
            + "InfiniteList<Integer> twos = ones.map(x -> x * 2)\n"
            + "After twos.tail().head()\n"
            + " ..ones.toString() returns [[1] [[1] ?]]",
        () -> ones.toString(),
        "[[1] [[1] ?]]");
    i.expect(" ..twos.toString() returns [[2] [[2] ?]]", () -> twos.toString(), "[[2] [[2] ?]]");
    twos.head();
    i.expect(
        " ..calling twos.head() again\n" + " ....causes zero evaluation of () -> 1",
        generateHistory,
        List.of(1, 1));
    i.expect(" ....causes zero evaluation of x -> x * 2", doublerHistory, List.of(1, 1));
    i.expect(
        " ..calling twos.tail().head() again\n" + " ....causes zero evaluation of () -> 1",
        generateHistory,
        List.of(1, 1));
    i.expect(" ....causes zero evaluation of x -> x * 2", doublerHistory, List.of(1, 1));
  }
