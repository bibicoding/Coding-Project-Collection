...
class Test4 {
  ...
  public static void main(String[] args) {
    ...
    i.expect(
        "Lazy<Integer> fifty = Lazy.of(5).combine(Lazy.of(10), (x, y) -> x * y);\n"
            + "fifty.toString() returns ?",
        fifty.toString(),
        "?");
    i.expect("fifty.get() returns 50", fifty.get(), 50);
    Lazy<Integer> hundred = fifty.combine(fifty, (x, y) -> x + y);
    i.expect("fifty.combine(fifty, (x, y) -> x + y).toString() returns ?", hundred.toString(), "?");
    i.expect("fifty.combine(fifty, (x, y) -> x + y).toString() returns ?", hundred.get(), 100);
    ...
    i.expect(
        "Lazy<String> str = Lazy.of(10).combine(Lazy.of(0.01), (x, y) -> x + \" \" + y)\n"
            + "str.toString() returns ?",
        str.toString(),
        "?");
    i.expect("s.toString() returns ? again", str.toString(), "?");
    i.expect("s.get() returns \"10 0.01\"", str.get(), "10 0.01");

    i.expectCompile(
        "Lazy<Number> n = Lazy.<String>of(\"hello\").combine(Lazy.<Integer>of(123), "
